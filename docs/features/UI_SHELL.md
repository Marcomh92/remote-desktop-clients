...
| `USELOCALCURSOR` | `CURSOR_AUTO=0` | `0=AUTO, 1=FORCE_LOCAL, 2=FORCE_DISABLE`. `AUTO` is flavor-dependent (see `RemoteCanvas.needsLocalCursor:374-395` and `docs/features/INPUT_PIPELINE.md` §1): VNC → server-pushed `XCursor`/`RichCursor` pixels fill `softCursor` directly via `Decoder.handleCursorShapeUpdate`; SPICE/Opaque → static `R.drawable.cursor` PNG (host-cursor callback is an un-implemented TODO stub in `remoteClientLib/src/main/cpp/android/android-spice-widget.c:314-319`); RDP → no static PNG seed; the host AND/XOR-mask bitmap is forwarded from FreeRDP via `RemoteCanvas.OnPointerEvent{New,Set}`, which the `needsLocalCursor` initial seed skips in favor of. End result: user sees the host-rendered context-dependent shape (resize, hand, grab, etc.). `FORCE_LOCAL` and `FORCE_DISABLE` preserve their meaning on every flavor. The RDP path requires `remoteClientLib/jni/libs/22_freerdp_add_cursor_callback.patch` to have been applied during the FreeRDP native build — without it the C-side `android_register_pointer` no-op stubs drop the bitmap and the host cursor never arrives. | `Constants.CURSOR_*` |
