...
**`MOUSE` no-op swallow (INV-031).** `RemoteExtraKeysHandler.sendKey:89-95` early-returns for `key.equals(ExtraKeysConstants.KEY_MOUSE)` before the `PRIMARY_KEY_CODES_FOR_STRINGS` lookup and the literal-char fallback path. Without this guard the mouse-icon tap would type "M O U S E" into the focused text field on the remote. The mouse button is a visual parity marker only; a follow-up round should wire it to a real left-click via `RemoteRdpPointer.leftButtonDown/Up`.
...
**Drawable resources.** Existing only: `mouse_icon.png` (`bVNC/src/main/res/drawable{,-ldpi,-hdpi}/mouse_icon.png`) and `ic_screen_black_48dp.xml` (`bVNC/src/main/res/drawable/ic_screen_black_48dp.xml`). The latter is the existing Material screen icon — not a perfect camera-in-dashed-box match for the MS RDP screenshot glyph, but the closest available existing resource.
...
- INV-031 (round 11, RDP-only) — The `MOUSE` cell is a visual placeholder only and MUST NOT send "MOUSE" as 5 literal characters to the remote. `RemoteExtraKeysHandler.sendKey:89-95` early-returns when `key.equals(ExtraKeysConstants.KEY_MOUSE)`, before the `PRIMARY_KEY_CODES_FOR_STRINGS` lookup and the literal-char fallback path. Without this guard, the mouse-icon tap would type "M O U S E" into the focused text field on the remote. A real left-click implementation (a follow-up round) MUST add a new dispatch path in `sendKey` rather than un-guarding the existing one. Non-RDP flavors are unaffected: `KEY_MOUSE` is never referenced outside `RdpExtraGridPanel.GRID_JSON`.
