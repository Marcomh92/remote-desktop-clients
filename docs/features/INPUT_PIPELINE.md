...
### 4.3.8 RDP "123" grid round-12 specifics (Number Keys toggle, 1.5× cells, IME-gated toggle reserved)
...
**`recomputeRdpViewport` `toggleReserved` math (`RemoteCanvasActivity.java:1843-1874`).** Round 12 reserves vertical space below the input area for the floating keyboard toggle button so it sits **underneath** the modifier row / extra-keys grid instead of overlapping the bottom-right corner. `toggleReserved = toggleH + bottomMarginPx + 8 dp` (the `+8 dp` is a visual gap, derived as `(int) (8 * density + 0.5)`). Three gates must all hold:
1. `lastImeHeightPx == 0` — IME is closed (when the IME is open the toggle sits behind it; reserving space would create a wasted 48 dp gap between the input area and the IME top).
2. `rdpInputAreaContainer.getVisibility() == VISIBLE` — input area is showing.
3. `keyboardToggleButton.getVisibility() == VISIBLE` and `toggleH > 0` — the toggle itself is on-screen with a measured height.

`usable = canvas.getRdpFullViewHeight() − lastImeHeightPx − containerH − toggleReserved` (with `< 0` clamped to `0`) drives `canvas.setVisibleDesktopHeight(usable)`. The container translation now reads `rdpInputAreaContainer.setTranslationY(-imeOverlap − toggleReserved)` so the input area slides up to clear the toggle as well as the IME. When the IME is open, `toggleReserved = 0` and the math collapses to round-3 behavior (`-imeOverlap`).
