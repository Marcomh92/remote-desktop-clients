...
`bVNC/src/main/java/com/iiordanov/bVNC/input/TouchInputHandlerTouchpad.java` extends the generic handler with three gestures that match the Microsoft RDP Android app, plus the RDP-only velocity-based pointer acceleration curve (see §6) and an RDP-only precise finger-tracking mode (see §6 / INV-029). They are gated by `setRdp(boolean)` (`:138-...`); non-RDP touchpad sessions keep the legacy behaviour.
...
**Precise finger tracking (round 10, RDP-only).** When `rdpPreciseTracking` is on, the touchpad applies gain exactly `1 / zoom` instead of the standard `sensitivity / displayDensity × cbrt(zoom) × curveGain`: `onScroll` move (`:313-321`), the `getX`/`getY` drag-delta branch (`:653-657`, `:693-697`; also covers double-tap-committed drags), `Flinger.run` (`:879-884`) and `EdgePinRepeater.run` (`:959-964`). Touchpad sensitivity, display density, the `cbrt(zoom)` curve, and `PointerAccelerationCurve` are all bypassed; sub-pixel carry is retained for move + drag and `getDelta` (`:732`) is unchanged. `setPointerPreciseTracking(boolean)` (`:201-208`) resets the curve EMA + carry on every toggle and stops any in-flight flinger / edge-pin repeater when enabling; `getPreciseZoom()` (`:225-228`) falls back to `1f` when `viewable.getZoomFactor()` is `<= 0`. Documented exceptions while precise mode is on: momentum fling still coasts after finger release (scaled `1/zoom`, not disabled), edge-pin hold keeps moving the cursor while the finger is pinned at a canvas edge, and edge-band viewport panning can shift the drawn offset — the remote-cursor offset itself stays invariant. See §6 and INV-029.
...
| `RdpExtraGridPanel` | `ExtraKeysView`-based grid. **Round 12**: panel now implements `ExtraKeysView.IExtraKeysView` (`:51`), and the static `GRID_JSON` is replaced by three JSON template fragments — `LEFT_SECTION_TEMPLATE` (`:67-76`) with `%s` placeholders for the right section + toggle label, `RIGHT_SECTION_SPECIAL` (`:83-87`, 4×3 nav cluster), `RIGHT_SECTION_NUMPAD` (`:95-99`, 4×3 numeric layout) — stitched into a single array by `rebuildGrid()` (`:148-168`) and passed to `extraKeysView.reload(...)`. `init` (`:126-140`) calls `extraKeysView.setRdpGridMode(true)` before `rebuildGrid()` so the post-layout pass applies 4 dp per-side margins and the 1.5× height-to-width cell ratio on the first reload (preserves the legacy VNC/SPICE/Opaque 2-row pager, which keeps the old zero-margin / aspect-ratio sizing — see INV-019). Layout (still **4 rows × 9 columns**, mirroring Microsoft RDP): row 1 `[F1, F2, F3, "", BKSP, /, HOME, ▲, PGUP]`; row 2 `[F4, F5, F6, screenshot, ENTER, *, ◄, "", ►]`; row 3 `[F7, F8, F9, "", "", -, END, ▼, PGDN]`; row 4 `[F10, F11, F12, "Number Keys" span:2, "", +, INS, DEL, Enter]`. The round-11 mouse-icon cell is gone — its slot is now an empty-string gap inside `LEFT_SECTION_TEMPLATE` (see INV-030); the empty after the span in row 4 keeps `+` aligned at col 6 so the right section starts at col 7 in all rows. **Toggle button (INV-032).** `RdpExtraGridPanel.onExtraKeyButtonClick:191-204` intercepts the post-alias `"NUMLK"` key (after `CONTROL_CHARS_ALIASES` resolves `NUMLOCK → NUMLK`) and calls `toggleNumpad()` (`:214-217`), which flips `numpadShown` and re-invokes `rebuildGrid()` so the right section swaps between `RIGHT_SECTION_SPECIAL` and `RIGHT_SECTION_NUMPAD`; the toggle label flips between `TOGGLE_LABEL_SPECIAL = "Number Keys"` and `TOGGLE_LABEL_NUMPAD = "Special keys"` to advertise what tapping it will do next, not the current state. `NUMLK` is excluded from `PRIMARY_REPETITIVE_KEYS` so long-press does not flicker the panel. All non-toggle keys delegate to an inner `RemoteExtraKeysHandler` (set up in `setRemoteKeyboard:180-189`). Two cell-shaping features land in `ExtraKeyButton` + `ExtraKeysView`: `KEY_ICON` (`ExtraKeyButton.java:50, 82-83, 199-203, 215-218`) — drawable resource name string resolved via `Resources.getIdentifier`; an `iconResId != 0` cell renders as an icon-only `MaterialButton` (`ExtraKeysView.reload:404-417`: `setIconResource` + `setIconGravity(ICON_GRAVITY_TEXT_START)` + `setIconPadding(0)` + `setGravity(Gravity.CENTER)` + `setIconSize(24 dp)`, text hidden). `KEY_SPAN` (`ExtraKeyButton.java:53, 88, 159, 205-208`) — int column count, default 1, floored to 1; `ExtraKeysView.reload:489-493` emits `GridLayout.spec(col, span, FILL, 1f)` instead of `spec(col, FILL, 1f)` when `span > 1` (the "Number Keys" cell uses `span:2`). Empty-string cells are gaps: `ExtraKeysView.reload:394` skips `getKey().isEmpty()` cells instead of rendering an un-tappable button — see INV-030. Arrow overrides (`▲ ◄ ▼ ►`) bypass the default `←→↑↓` to match Microsoft RDP's triangle arrowheads. The round-11 mouse-icon cell is gone — see INV-031 for the deletion history. New wire paths: `SCREENSHOT → SYSRQ → KeyEvent.KEYCODE_SYSRQ → RdpKeyboardMapper.keymapAndroid[120] = VK_PRINT`; `NUMLOCK → NUMLK → KeyEvent.KEYCODE_NUM_LOCK → keymapAndroid[143] = VK_NUMLOCK` (`ExtraKeysConstants.java:75-76` + `:233-236`; `RdpKeyboardMapper.java:308, 325`). `PRIMARY_REPETITIVE_KEYS` (`ExtraKeysConstants.java:38-50`) expanded from 8 to 22 entries: added F1–F12, ENTER, HOME, END, and arithmetic operators `/ * - +`; `SYSRQ` / `NUMLK` / `INS` excluded so toggles and destructive keys never auto-repeat. Round 5 sizing fix still applies: `init:137-139` inserts the inner `ExtraKeysView` with `MATCH_PARENT × MATCH_PARENT` `LinearLayout.LayoutParams` so the GridLayout's FILL rowSpecs distribute the parent height. Backward compat: `ExtraKeysPagerAdapter.java:106-122` (legacy VNC/SPICE/Opaque pager) still uses the 2-row JSON with no `icon`/`span`/empty cells, so the new code paths are dormant for non-RDP flavors. See `bVNC/src/main/java/com/iiordanov/bVNC/extrakeys/RdpExtraGridPanel.java:51-218`. |
...
### 4.3.7 RDP "123" grid alignment with Microsoft RDP (round 11)

`RdpExtraGridPanel.GRID_JSON` (`bVNC/src/main/java/com/iiordanov/bVNC/extrakeys/RdpExtraGridPanel.java:60-72`) was rewritten from the legacy 3-row × 8-col layout to a **4-row × 9-col** layout that mirrors the Microsoft RDP Android app's "123" page. The grid is the RDP-only entry point — `ExtraKeysPagerAdapter.java:106-122` (legacy VNC/SPICE/Opaque pager) is byte-identical and still uses the 2-row JSON without `icon`/`span`/empty cells, so the new code paths are dormant for non-RDP flavors.

**Layout (4 rows × 9 columns).** Row 1 `[F1, F2, F3, mouse-icon, BKSP, /, HOME, ▲, PGUP]`; row 2 `[F4, F5, F6, screenshot-icon, ENTER, *, ◄, (empty), ►]`; row 3 `[F7, F8, F9, (empty), (empty), -, END, ▼, PGDN]`; row 4 `[F10, F11, F12, "Number Keys" wide (span:2), (empty), +, INS, DEL, Enter]`. Cells whose key is the empty string render as gaps (see INV-030). Arrow cells use explicit `display: "▲"/"◄"/"▼"/"►"` overrides (the `ARROW_TRIANGLE_VARIATION_DISPLAY` glyphs in `ExtraKeysConstants.java:136-142`) instead of the default `←→↑↓` to match Microsoft RDP's triangle arrowheads.

**Two new JSON cell-shaping features** in `ExtraKeyButton` (`bVNC/src/main/java/com/iiordanov/bVNC/extrakeys/ExtraKeyButton.java`):

| JSON key | Type | Default | Where it is read |
|---|---|---|---|
| `icon` | String (drawable resource name) | absent → no icon | `getIconResId(Context)` (`ExtraKeyButton.java:215-218`) → `Resources.getIdentifier`. `ExtraKeysView.reload:404-417` renders an icon-only `MaterialButton` (`setIconResource` + `setIconGravity(ICON_GRAVITY_TEXT_START)` + `setIconPadding(0)` + `setGravity(Gravity.CENTER)` + `setIconSize(24 dp)`, text hidden) when `iconResId != 0`; otherwise the cell renders the display text as before. |
| `span` | Int (column count) | `1`, floored to `1` (`ExtraKeyButton.java:151-159`) | `ExtraKeysView.reload:489-493` emits `GridLayout.spec(col, span, FILL, 1f)` when `span > 1` instead of the single-column `spec(col, FILL, 1f)`. Used by the "Number Keys" cell (`NUMLOCK` key, `display: "Number Keys"`, `span: 2`). |

**Empty-cell handling (INV-030).** `ExtraKeysView.reload:394` (`if (buttonInfo.getKey().isEmpty()) continue;`) skips the entire cell construction, so a JSON `""` produces no `MaterialButton` and the column position remains visually empty. With `columnCount = maximumLength(buttons) = 9` (set at `ExtraKeysView.reload:385`), this is what makes row 2 col 8, row 3 cols 4-5, and row 4 col 5 land as gaps while the rest of the row stays filled.

**New wire paths.** Two new keycodes land in `ExtraKeysConstants.PRIMARY_KEY_CODES_FOR_STRINGS:75-76`: `SYSRQ → KeyEvent.KEYCODE_SYSRQ` (screenshot/Print Screen, mapped to `VK_PRINT` by `RdpKeyboardMapper.keymapAndroid[120] = VK_PRINT` at `RdpKeyboardMapper.java:308`) and `NUMLK → KeyEvent.KEYCODE_NUM_LOCK` (mapped to `VK_NUMLOCK` by `keymapAndroid[143] = VK_NUMLOCK` at `:325`). Four new aliases in `CONTROL_CHARS_ALIASES:233-236`: `PRINT → SYSRQ`, `PRINT_SCREEN → SYSRQ`, `SCREENSHOT → SYSRQ`, `NUMLOCK → NUMLK`. The "Number Keys" button's "switch to numeric-pad layout" behavior is a placeholder today — the tap only sends `KEYCODE_NUM_LOCK`; full layout switching is out of scope for this round.

**Drawable resources.** Existing only: `ic_screen_black_48dp.xml` (`bVNC/src/main/res/drawable/ic_screen_black_48dp.xml`) — the existing Material screen icon, not a perfect camera-in-dashed-box match for the MS RDP screenshot glyph but the closest available existing resource. `mouse_icon.png` is no longer referenced by any Java source after round 12 (the file can be deleted in a future cleanup round).

### 4.3.8 RDP "123" grid round-12 + round-13b specifics (Number Keys toggle, 1.5× cells, z-order-under toggle, multi-line labels, INS-repeat)

Round 12 turns the static round-11 `GRID_JSON` into a rebuildable grid plus an IME-aware toggle reservation. Five things changed.

**`LEFT_SECTION_TEMPLATE` JSON shape (`:67-76`).** The left half of every row is one `LEFT_SECTION_TEMPLATE` fragment with `%s` placeholders for the right section + toggle label. The empty-string cells in rows 1, 2, 3 of col 4 are deliberate placeholders (the round-11 mouse-icon slot, see INV-030); the empty after `NUMLOCK`'s `span:2` in row 4 anchors `+` at col 6 so the right section uniformly starts at col 7 in all rows (per the source comment at `:74-75`). The `LEFT_SECTION_TEMPLATE` itself never references round-11 `MOUSE` — round 12 dropped the cell, not just the constant.

**`mRdpGridMode` and the post-layout `OnLayoutChangeListener` (`ExtraKeysView.java:181-204, 218-249`).** Two new constants `BUTTON_HEIGHT_TO_WIDTH_RATIO = 1.5f` and `CELL_MARGIN_DP = 4f`, a `mRdpGridMode` boolean (default `false`) and a `setRdpGridMode(boolean)` setter (`:407-411`) gate an RDP-only post-layout pass. On every layout change the listener derives `marginPx = (CELL_MARGIN_DP × density + 0.5)`, `cellWidth = (width − colCount × 2 × marginPx) / colCount`, and `targetHeight = cellWidth × BUTTON_HEIGHT_TO_WIDTH_RATIO`; it overrides each child's `lp.height` to `targetHeight` and re-issues `requestLayout()` only when a height actually changed (the `lp.height == targetHeight` guard against re-entry). The legacy 2-row VNC/SPICE/Opaque pager is allocated a fixed 68 dp height (see `dimens.xml extra_keys_height`) and would overflow if every button demanded 1.5× its column width — that is why `mRdpGridMode` defaults to `false` and is only flipped by `RdpExtraGridPanel.init:131`.

**`ExtraKeysView.reload`'s gated per-cell margin (`:556-561`).** `param.setMargins(cellMarginPx, cellMarginPx, cellMarginPx, cellMarginPx)` where `cellMarginPx = mRdpGridMode ? (CELL_MARGIN_DP × density + 0.5) : 0`. Same `setRdpGridMode(true)` requirement: `RdpExtraGridPanel.init` must call it before the first `rebuildGrid()` so the initial cell layout has the margins; the post-layout pass works regardless of call order because it re-reads `mRdpGridMode` every frame.

**`RdpExtraGridPanel.onExtraKeyButtonClick` NUMLK interceptor + post-alias key (`:191-204`).** The panel implements `ExtraKeysView.IExtraKeysView` (`:51`) so it sits **between** the `ExtraKeysView` and the inner `RemoteExtraKeysHandler`. Every click hits the panel first; the first thing it checks is `if ("NUMLK".equals(buttonInfo.getKey()))`. `"NUMLK"` is the **post-alias** value — `CONTROL_CHARS_ALIASES` (`ExtraKeysConstants.java:233-236`) maps `NUMLOCK → NUMLK`, so by the time the click handler runs the cell's `getKey()` is already `"NUMLK"` (not `"NUMLOCK"`). Matching the literal `"NUMLK"` is what prevents the literal-char dispatch path from typing `N U M L K` into a focused text field; matching `"NUMLOCK"` would miss because the alias has already been resolved. On match the panel calls `toggleNumpad()` and `return`s — no key event reaches the delegate. Non-toggle keys fall through to `delegate.onExtraKeyButtonClick(...)` (`:201-203`); a null keyboard (pre-connection) makes the panel inert. INV-032 governs the local-only semantics. `NUMLK` is excluded from `PRIMARY_REPETITIVE_KEYS` (`ExtraKeysConstants.java:38-50`) so long-press does not flicker the panel.

**`rebuildGrid()` + `toggleNumpad()` (`:148-168, :214-217`) and `RIGHT_SECTION_SPECIAL` / `RIGHT_SECTION_NUMPAD` (`:83-99`).** `rebuildGrid()` picks `RIGHT_SECTION_NUMPAD` when `numpadShown == true` else `RIGHT_SECTION_SPECIAL`, picks the matching `TOGGLE_LABEL_*` (`:56-59`), formats `LEFT_SECTION_TEMPLATE` with five `%s` slots (row 1/2/3 right section + row 4 toggle label + row 4 right section), wraps in `[ ]`, and passes the JSON to `extraKeysView.reload(info, 0)`. `toggleNumpad()` flips `numpadShown` and re-invokes `rebuildGrid()` — the right section swaps between the 4×3 nav cluster (`HOME/▲/PGUP` / `◄/ /►` / `END/▼/PGDN` / `INS/DEL/Enter`) and the 4×3 numeric layout (`1/2/3` / `4/5/6` / `7/8/9` / `0/./ENTER`). The toggle label flips between `TOGGLE_LABEL_SPECIAL = "Number Keys"` (special-keys mode — what tapping will switch TO) and `TOGGLE_LABEL_NUMPAD = "Special keys"` (numpad mode), so the same cell always advertises what tapping it will do next.

**`recomputeRdpViewport` no longer reserves space for the floating keyboard toggle (`RemoteCanvasActivity.java:1818-1855`).** Round 12 reserved `toggleReserved = toggleH + bottomMarginPx + 8 dp` so the toggle visually sat underneath the input area; round 13b removed that math and replaced it with a pure layout fact — `FrameLayout` z-order in `canvas.xml` (and `layout-large/canvas.xml`) declares `<ImageButton android:id="@+id/keyboardToggleButton" ... />` BEFORE `<FrameLayout android:id="@+id/rdpInputAreaContainer" ... />`, so the container is painted on top. The current math is `usable = canvas.getRdpFullViewHeight() − lastImeHeightPx − containerH` (with `< 0` clamped to `0`) — no `-toggleReserved` term — and `rdpInputAreaContainer.setTranslationY(-imeOverlap)` — no `-toggleReserved` translation. The input area sits flush at the screen bottom; the toggle hides behind it whenever the container is `VISIBLE` and re-emerges when the container is `GONE` (state `NONE`, see INV-018) because `FrameLayout` does not route touches to `GONE` views. Round 12's three-gate condition (`lastImeHeightPx == 0` AND container `VISIBLE` AND toggle `VISIBLE` AND `toggleH > 0`) is gone with the math.

**Round-13b multi-line labels + INS-repeat (`ExtraKeysConstants.java:46-53`, `RdpExtraGridPanel.java:62-75`, `ExtraKeysView.reload:498-501`).** Word labels in `RIGHT_ROW_SPECIAL` (HOME / PGUP / END / PGDN / INS / DEL / ENTER) wrap onto two lines via the `{key, display}` JSON dict form with an explicit `\n` in `display` (e.g. `{"key":"HOME","display":"Ho\nme"}` → "Ho" + "me"). `ExtraKeysView.reload:498-501` calls `setSingleLine(false) + setMaxLines(2)` on every button when `mRdpGridMode == true`; arrows are single-glyph and unaffected. The dict form is mandatory for any wrapped label — bare-string entries with `\n` would put the newline into the dispatch key, which `RemoteExtraKeysHandler.sendKey` would then miss in `PRIMARY_KEY_CODES_FOR_STRINGS` and fall through to the literal-char branch (typing `H o ENTER m e` into the remote session). `INS` joins `PRIMARY_REPETITIVE_KEYS` at `ExtraKeysConstants.java:48` so holding the Insert button repeats at the long-press cadence; `SYSRQ` and `NUMLK` remain excluded (NUMLK is the panel-mode toggle, SYSRQ is a discrete action).
...
| **`rdpPreciseFingerTracking`** (round 10, RDP-only) | `SwitchPreferenceCompat` — RDP-only precise finger tracking: cursor follows the finger 1:1 (`remoteDelta = viewDelta / zoom`), bypassing touchpad sensitivity, display density and the `PointerAccelerationCurve`. While ON, the four bypassed rows (`touchpadSensitivity`, `rdpPointerAccelEnabled`, `rdpPointerAccelLowGainPct`, `rdpPointerAccelHighGainPct`) are greyed out by `GlobalPreferencesFragment.applyPreciseTrackingGreying` (`:90-107`); the `rdpPointerAccelReset` action stays enabled. | `false` | `global_preferences_rdp.xml:15-19`, `Constants.rdpPreciseFingerTracking` / `DEFAULT_RDP_PRECISE_FINGER_TRACKING`, `RemoteCanvasActivity.getRdpPreciseFingerTracking:1389-1391`; pushed via `TouchInputHandlerTouchpad.setPointerPreciseTracking:201-208` |
...
- INV-019 — All RDP-only UX elements (`ModifierRowView`, `RdpExtraGridPanel`, `RdpModifierRowHandler`, `rdpInputAreaContainer`, `recomputeRdpViewport`, the IME insets listener, RDP touchpad gestures, the round-5 `movePanToMakePointerVisible()` call inside `Flinger.run:667`, the round-5 "123" grid `MATCH_PARENT` insert in `RdpExtraGridPanel.init:93-95`, the round-5 double-back disconnect in `onBackPressed:1808-1817`, the round-6 modifier row sizing prefs `rdpModifierKeyHeightDp` / `rdpModifierKeySizeDp` and their `applyModifierRowSizing` push sites, the round-6 Alt+Tab cycling fix in `RemoteRdpKeyboard.fireKeyDispatchedIfApplicable:127-131` and `processLocalKeyEvent:75-90`, the round-6 toggle-OFF `releaseOnScreenModifiers` / `releaseModifierKeys` path, **round 9**: `PointerAccelerationCurve` (the `isRdp` gate inside `TouchInputHandlerTouchpad.getDelta`), the `rdpPointerAccel*` settings + the `rdpPointerAccelReset` action, `sendPointerEvent`'s MOVE-dedup guard (RDP-only — VNC/SPICE pointers untouched), and `DoubleTapPairTracker` integration via the `TouchInputHandlerTouchpad` `isRdp` guard inside `onManualDoubleTap`'s override, the round-10 precise finger-tracking mode (`rdpPreciseTracking` + the `rdpPreciseFingerTracking` pref + `setPointerPreciseTracking`), and **round 11**: the 4×9 `RdpExtraGridPanel.GRID_JSON`, the `ExtraKeyButton.KEY_ICON`/`KEY_SPAN` JSON shaping features, the icon-only button render in `ExtraKeysView.reload:404-417`, the `GridLayout.spec(col, span, ...)` path in `ExtraKeysView.reload:489-493`, the empty-cell gap skip in `ExtraKeysView.reload:394`, the `SYSRQ`/`NUMLK` keycodes + `SCREENSHOT`/`NUMLOCK` aliases in `ExtraKeysConstants.java:34, 75-76, 233-236`, and **round 12**: the `BUTTON_HEIGHT_TO_WIDTH_RATIO` (`1.5f`) and `CELL_MARGIN_DP` (`4f`) constants plus the `mRdpGridMode` field and `setRdpGridMode(boolean)` setter in `ExtraKeysView` (RDP-only — the legacy VNC/SPICE/Opaque 2-row pager stays at zero margin and aspect-ratio sizing), the post-layout `OnLayoutChangeListener` in `ExtraKeysView` (derives `cellWidth = (width − colCount × 2 × marginPx) / colCount` and overrides each button's height to `cellWidth × BUTTON_HEIGHT_TO_WIDTH_RATIO`, with a `lp.height == targetHeight` guard against re-entry), the gated per-cell margin in `ExtraKeysView.reload` (0 when `!mRdpGridMode`, 4 dp when `mRdpGridMode`), the `RdpExtraGridPanel` rewrite — now implements `ExtraKeysView.IExtraKeysView` and replaces the static `GRID_JSON` with `LEFT_SECTION_TEMPLATE` + `RIGHT_SECTION_SPECIAL` / `RIGHT_SECTION_NUMPAD` JSON fragments stitched by `rebuildGrid()` (toggle label flips between `"Number Keys"` and `"Special keys"`), the local `NUMLK` intercept in `RdpExtraGridPanel.onExtraKeyButtonClick:191-204` that runs before the delegate (see INV-032), the `PRIMARY_REPETITIVE_KEYS` expansion from 8 to 22 entries (added F1–F12, ENTER, HOME, END, and arithmetic operators; `SYSRQ`/`NUMLK`/`INS` excluded so toggles / destructive keys never repeat), and the round-12 `recomputeRdpViewport` change that reserved `toggleReserved = toggleH + bottomMarginPx + 8 dp` (gated on `lastImeHeightPx == 0` AND both views `VISIBLE`; repositioned via `setTranslationY(-imeOverlap − toggleReserved)`); round 13b removes this entirely — the toggle sits under the input area via a `FrameLayout` z-order swap in `canvas.xml` (and `layout-large/canvas.xml`) where `<ImageButton android:id="@+id/keyboardToggleButton" ... />` is declared BEFORE `<FrameLayout android:id="@+id/rdpInputAreaContainer" ... />`, so the container paints on top; `recomputeRdpViewport` math collapses to `usable = full − ime − containerH` and `setTranslationY(-imeOverlap)`. Round 13b also adds the `{key, display}` JSON dict form with explicit `\n` in `display` for the seven wrapped word labels in `RdpExtraGridPanel.RIGHT_ROW_SPECIAL` (HOME / PGUP / END / PGDN / INS / DEL / ENTER -> "Ho"+"me" / etc.), with `ExtraKeysView.reload:498-501` calling `setSingleLine(false) + setMaxLines(2)` on each button when `mRdpGridMode == true`; `INS` joins `ExtraKeysConstants.PRIMARY_REPETITIVE_KEYS` (line 48)) are gated by `Utils.isRdp(this)` or by the `TouchInputHandlerTouchpad` `setRdp(true)` guard. **Round-7 carve-out:** `keyboardToggleButton` and `toolbarToggleButton` FABs are visible on every flavor — they are NOT RDP-only. The keyboard FAB click handler (`onKeyboardToggleButtonClicked:1757-1773`) still branches on `Utils.isRdp(this)` (RDP routes into `RdpModifierRowHandler.onBackToKeyboard`; non-RDP calls `showKeyboard` / `hideKeyboard`). Non-RDP flavors (bVNC/aSPICE/Opaque) must remain byte-identical to the pre-RDP UX for the RDP-only subsystems — the `ExtraKeysPagerAdapter` JSON (`ExtraKeysPagerAdapter.java:106-122`) does not use `icon`/`span`/empty cells, so the round-11 code paths are dormant for them.
...
- INV-029 (round 10, RDP-only) — Precise finger tracking. When `isRdp && rdpPreciseTracking` (default `false`), the touchpad applies gain exactly `1 / zoom` (`zoom = viewable.getZoomFactor()`, `1f` fallback when `zoom <= 0`) and bypasses touchpad sensitivity, `/displayDensity`, `cbrt(zoom)`, and the `PointerAccelerationCurve` in `onScroll` move, the `getX`/`getY` drag-delta branch (including double-tap-committed drags), `Flinger.run`, and `EdgePinRepeater.run`. Sub-pixel carry is retained for move + drag; `getDelta` is unchanged. With the toggle OFF (or on non-RDP sessions) every path is byte-identical to the round-9 behavior. `setPointerPreciseTracking(boolean)` resets the curve EMA + carry on every toggle and stops in-flight flinger / edge-pin repeaters when enabling. Non-RDP flavors must remain byte-identical. Device verification pending (no unit tests were added — `:bVNC` has no test source set).
- INV-030 (round 11, RDP-only) — Empty-string JSON cells in `ExtraKeysView.reload` render as gaps, not as tappable buttons. `ExtraKeysView.java:394` (`if (buttonInfo.getKey().isEmpty()) continue;`) skips the entire cell construction, so a JSON `""` produces no `MaterialButton` and the column position remains visually empty. This is what enables row 2 col 8, row 3 cols 4-5, and row 4 col 5 to land as gaps while the rest of the row stays filled, given `columnCount = maximumLength(buttons) = 9`. Adding a non-empty cell to a position that was previously empty must preserve this rule (no partially-rendered button); removing all cells from a row must preserve the rowSpec (GridLayout still emits one FILL row). VNC/SPICE/Opaque JSON (`ExtraKeysPagerAdapter.java:106-122`) has no empty cells, so the rule is dormant for those flavors.
- INV-031 (round 11, **amended round 12**, RDP-only) — Round 11 had a `MOUSE` visual-placeholder cell; the round-12 rewrite drops the cell entirely (the slot is now an empty-string gap inside `LEFT_SECTION_TEMPLATE`, see INV-030). With the cell gone the round-11 `MOUSE` constant in `ExtraKeysConstants` is no longer needed and was **deleted** in round 12; the `RemoteExtraKeysHandler.sendKey` early-return that swallowed it is gone with it. A future left-click implementation (a round-13 follow-up) MUST add a new dispatch path in `sendKey` for a fresh key id rather than re-introducing the round-11 `MOUSE` constant. Non-RDP flavors are unaffected — they never referenced the round-11 cell.
- INV-032 (round 12, RDP-only) — `RdpExtraGridPanel` intercepts the **post-alias** key `"NUMLK"` (the value after `CONTROL_CHARS_ALIASES` resolution, see `RdpExtraGridPanel.java:191-204`) and uses it as a local-UI-only toggle: a click flips `mNumpadMode` and calls `rebuildGrid()`, no key event reaches the remote. The toggle label flips between `TOGGLE_LABEL_SPECIAL = "Number Keys"` (special-keys mode) and `TOGGLE_LABEL_NUMPAD = "Special keys"` (numpad mode) so the same cell advertises what tapping it will do next, not the current state. `RemoteExtraKeysHandler.onExtraKeyButtonClick` MUST NOT see `"NUMLK"` for the toggle to stay local — the panel's interceptor runs **before** the delegate, and `RemoteExtraKeysHandler`'s own `NUMLK` lookup still produces a real `KEYCODE_NUM_LOCK` send when invoked from a non-RDP caller (legacy VNC/SPICE pager uses `NUMLOCK` without the alias).
