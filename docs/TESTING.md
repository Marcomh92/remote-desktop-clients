...
| **Double back-press disconnects** (round 5: `RemoteCanvasActivity.onBackPressed:1808-1817`) | Time-window logic is per-instance; `SystemClock.uptimeMillis()` is non-deterministic in tests. | Manual smoke. Cover: from a connected session with no IME visible, first back-press shows the "Press back again to disconnect" toast and does NOT forward to the remote; second back-press within 2 s disconnects the session; waiting > 2 s and pressing back again re-arms (first press is again a no-op with a fresh toast). Confirm `Utils.isRdp(this)` gating: open bVNC / aSPICE / Opaque, press back, and confirm it still forwards as KEYCODE_BACK. Confirm TV devices still immediately disconnect on the first press. **Round 6 caveat**: confirm on an Android 13+ device with predictive-back gestures enabled — the manifest opt-out (ADR-0002) must keep `onBackPressed` reachable, AND `RemoteClientsInputListener.kt:60-62` must short-circuit BACK to `return false` on RDP/TV so the View does not consume the press before the Activity sees it; without either, the round-5 toast never appears. |
...
| **Floating keyboard-toggle button visual** (round 5 restyle + round 6 visual delta + round 7 flavor coverage: `canvas.xml:155-166` + `bg_keyboard_toggle.xml`) | Static layout / drawable change — visual only. | Manual smoke. Cover: on **every flavor** (bVNC / aRDP / aSPICE / Opaque — round 7), confirm the button shows up as a 32 dp × 32 dp rounded-rect (10 dp corners, ≈ 20 % black) with the hamburger icon (Material `ic_baseline_menu_48`); drag-vs-tap still works (drag moves the button; on RDP tap cycles `NONE → KEYBOARD → EXTRA → NONE` via `onKeyboardToggleButtonClicked:1757-1773`, on non-RDP tap toggles `showKeyboard` / `hideKeyboard`); on RDP the round-7 `Utils.isRdp(this)` branch should land on the IME via `rdpModifierRowHandler.onBackToKeyboard()` and the extra-keys grid (if open) should collapse. Confirm `canvas.xml` and `layout-large/canvas.xml` stay byte-identical. |
| **Floating toolbar-toggle button** (round 7: `canvas.xml:168-178` + `ic_overflow_vertical_24.xml` + drag-vs-tap persistence via `USELASTPOSITIONTOOLBAR_X`/`_Y`/`_MOVED`) | New FAB + removed `moveToolbar` menu item + new layout XML; verification crosses the FAB-to-toolbar anchor math, the off-thread DB write, and the layout-pass re-apply. | Manual smoke on every flavor. Cover: at session start the FAB appears at `right|center` (or `left|center` under `leftHandedModeTag`) — the gravity mirror in `RemoteCanvasActivity.continueConnecting:477-485` is the only time the FAB moves without user input. Confirm the toolbar (`@+id/toolbar`) starts hidden (default `visibility="gone"` on `<com.undatech.opaque.util.RemoteToolbar>`). Tap the FAB → toolbar expands anchored next to the FAB (preferred left, fallback right, clamped to canvas bounds via `positionToolbarNextToToggle:1859-1901`). Tap again → toolbar collapses. With the IME open → FAB tap is a no-op. Drag the FAB past `scaledTouchSlop` → FAB moves with the finger; release past slop → position saved to DB (verify the SQLite row's `USELASTPOSITIONTOOLBAR_X` / `_Y` / `_MOVED` columns change; verify the update happens off the UI thread via `handler.post` in `saveToolbarTogglePosition:1928-1938`). Drag back into bounds, close & reopen the connection → FAB restores to the saved X/Y. Rotate the device after dragging the FAB off-screen → FAB falls back to its gravity default (out-of-bounds saves are silently dropped by `restoreToolbarTogglePosition:1910-1926`). Confirm `canvas.xml` and `layout-large/canvas.xml` stay byte-identical, and that the `RemoteToolbar` subclass is still on the classpath (no compile regression in `:remoteClientLib`). |
...
| **RDP precise finger tracking** (round 10: `TouchInputHandlerTouchpad.rdpPreciseTracking` — `onScroll` / `getX` / `getY` / `Flinger.run` / `EdgePinRepeater.run` all bypass sensitivity + density + curve and apply gain `1/zoom`) | Touch event sequencing is not expressible as plain JUnit; `:bVNC` has no `src/test/` source set and no Robolectric dependency, so no unit tests were added (out of scope). | **Device verification pending.** Manual smoke on a real RDP device: enable *Precise finger tracking* in Settings → confirm the sensitivity + acceleration rows grey out (the reset action stays enabled); move the finger slowly and fast → the on-screen finger-to-cursor offset stays constant (no acceleration, no density scaling); pinch-zoom then move → the offset still matches the on-screen distance at any zoom; disable the toggle → round-9 acceleration feel returns. Confirm non-RDP flavors (bVNC/aSPICE/Opaque) are unchanged (the setting is not shown there and `rdpPreciseTracking` stays false). Documented exceptions while enabled: fling still coasts after release, edge-pin still moves the cursor at a screen edge, and edge-band viewport panning can shift the drawn offset. |
