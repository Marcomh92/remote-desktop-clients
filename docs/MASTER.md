...
| **`RdpViewport` log tag** | Dedicated `adb logcat -s RdpViewport:V` channel for BUG-002 (cursor travels under the soft keyboard). Emits on the RDP IME insets listener, `recomputeRdpViewport`, `setInputAreaState`, the legacy 19% `relayoutViews` heuristic, `itemCenterMouse`, the `setVisibleDesktopHeight` / `setRdpFullViewHeight` setters, `movePanToMakePointerVisible`, and the cursor-position paths in `RemotePointer` (`movePointer`, `movePointerToMakeVisible`, `setY`, `setNewPointerPosition`). Per-field-change dedup keeps the stream readable; cursor movement is throttled to a 8-framebuffer-pixel delta. |
